 ...
 *  1) every Session manage a TcpClient
 *  2) statistics of throughput for every TcpClient
 */
...
private:
  ...
  TcpClient client_; ///< client
  Client* owner_; ///< get msg, call onConnect() and onDisconnect()
  int64_t bytesRead_;
 ...
 *  1) manage all connections
 *  2) onConnect() onDisconnect
 *  3) timeout handle
 *  4) has message
 *  5) statistics of throughput for all TcpClient
 */
 ...
 * arg[1]  host_ip   ip
 * arg[2]  port      port
 * arg[3]  threads   every thread own a EventLoop object, all TcpClient is
 *                   cycled sequential aligned into threads.
 * arg[4]  blocksize byte count of msg sent
 * arg[5]  sessions  TcpClient count
 * arg[6]  time      over test after \time seconds
*/
